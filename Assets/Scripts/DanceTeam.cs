...
public class DanceTeam : MonoBehaviour
{
    ...
    //health of whole team
    public int teamHealth;
    public int maxTeamHealth = 50;

    private void Start()
    {
        teamHealth = maxTeamHealth;
    }
    ...
    private void Update()
    {
        ...
        if (teamHealth <= 0)
        {
            Debug.Log("Dancer Of the " + danceTeamName + ":  died");
            teamHealth = maxTeamHealth;
            for (int i = activeDancers.Count - 1; i > 0; i--)
            {
                transform.Find("Dancer(Clone)").GetComponent<Character>().health = 0;
            }
        }
    }
